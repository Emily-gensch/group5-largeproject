...
const SearchPage = () => {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        const response = await fetch('http://https://lighthearted-moxie-82edfd.netlify.app/api/displayMovies', {
          method: 'POST',
          headers: {
  ...
  const handleSearch = async () => {
    if (searchTerm === '') {
      ...
      try {
        const response = await fetch('http://https://lighthearted-moxie-82edfd.netlify.app/api/searchMovie', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const handleAddToVote = async (movieId) => {
    ...
    try {
      const response = await fetch('http://https://lighthearted-moxie-82edfd.netlify.app/api/poll/addMovieToPoll', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ partyID: '', movieID: movieId }), // needs work
      });
      ...
      // doesnt work yet supposed to remove 
